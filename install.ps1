...
VBoxManage createhd --filename "$HOME/VirtualBox VMs/$($config.VMName)/$($config.VMName).vdi" --size $config.VDISize --format VDI

VBoxManage storagectl $config.VMName --name "SATA Controller" --add sata --controller IntelAhci

VBoxManage storageattach $config.VMName --storagectl "SATA Controller" --port 0 --device 0 --type hdd --medium "$HOME/VirtualBox VMs/$($config.VMName)/$($config.VMName).vdi"
